...
export const UPSTREAM_MODULE_MANIFEST: readonly UpstreamModuleManifestEntry[] = [
  ...
  },
  {
    ...
    status: "done",
    notes: "Binary ICC low-level parsing helpers, raw/cooked tag handling, linked tags, memory profile save/open, generic stream save, and ICC header/tag-table serialization are in place for the current runtime-neutral scope; raw/link/save semantics are differential-tested against upstream on the bundled corpus.",
  },
  {
    ...
    status: "done",
    notes: "Supported tag payload serialization, profile info lookup, intent/LUT selection, devicelink lookup, float DToB/BToD selection, named-color pipeline selection, Lab v2/v4 compatibility stages, matrix-shaper fallbacks, and interpolation-policy glue are in place for the current scope; parse/selection/save-reparse behavior is differential-tested against upstream on tmp/Little-CMS testbed and the bundled icc-profiles corpus.",
  },
  {
    ...
    status: "done",
    notes: "Pipeline/stage data model and LUT tag mapping from mft1/mft2/mAB/mBA/mpet are in place, including float CLUT evaluation, named-color stages, Lab/XYZ normalization compatibility helpers, upstream-compatible mAB/mBA stage ordering, and raw-preserving generic mpet handling for unknown/vendor elements; current behavior is differential-tested against upstream on the bundled corpus.",
  },
  {
