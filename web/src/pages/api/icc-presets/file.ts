...
export default async function handler(req: ApiRequest, res: ApiResponse): Promise<void> {
  ...
  if (!relativePath) {
    res.status(400).json({ error: "Invalid ICC preset path" });
    return;
  }

  const absolutePath = path.resolve(ICC_ROOT, relativePath);
  if (!absolutePath.startsWith(ICC_ROOT)) {
    res.status(400).json({ error: "Invalid ICC preset path" });
    return;
  }
